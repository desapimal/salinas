...
##=====Solo se modifica el UMASK====## 

echo "=====Editando /etc/login.defs	SOLO UMASK====="

#echo "  [ /etc/login.defs ]        "
#echo "  [ Editando lineas: PASS_MAX_DAYS | PASS_MIN_DAYS | PASS_MIN_LEN | PASS_WARN_AGE ] "

sed -i s/"UMASK	[0-9]*$"/"UMASK           022"/	/etc/login.defs
...
services=(cups sendmail bluetooth hplip postfix autofs atd ip6tables rhnsd rpcgssd rpcsvcgssd rhsmcertd rlogin rsh rexec )
...
sed -i s/".PubkeyAuthentication [a-z]*$"/"PubkeyAuthentication no"/g /etc/ssh/sshd_config
sed -i s/"PubkeyAuthentication [a-z]*$"/"PubkeyAuthentication no"/g /etc/ssh/sshd_config
echo " "
echo "			=====> Listo <====="
echo " "
...
##=====Se omiten el forzado de pwd alfanumerico debido a que causa problemas como lo ocurrido con digitalizacion====##
##=====Recordar las ultimas 5 contrasenas, si se ejecuta====##

#echo "  [ Recordar ultimas 5 Contrasenas]        "
#sed -i '/password    sufficient    pam_unix.so/d' /etc/pam.d/system-auth-ac
#sed -i '14i password    sufficient    pam_unix.so md5 shadow nullok try_first_pass use_authtok remember=5' /etc/pam.d/system-auth-ac		      
echo " "
echo "                  =====> Listo <====="
echo " "
...
##==Agrega los usuarios permitidos a los servers==##
